...
namespace poem {
  ...
  class Dimension : public Named {
   ...
   public:
    Dimension(const std::string &name,
              ...
              const std::string &description) : Named(name, unit, description,
                                                      type::DOUBLE) {} // A dimension has always the type DOUBLE (interpolation coord)
  };

  template<size_t _dim>
  class DimensionSet {
   public:
    explicit DimensionSet(const std::array<std::shared_ptr<Dimension>, _dim> &dimension_array) : m_dimension_array(
        dimension_array) {
  ...
  class DimensionPointSet : public DimensionPointSetBase {
   public:
    explicit DimensionPointSet(const DimensionGrid<_dim> &dimension_grid) : m_dimension_points(
        dimension_grid.dimension_points()),
