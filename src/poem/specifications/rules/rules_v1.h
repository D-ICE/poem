...
namespace poem::v1 {
  ...
  inline bool check_dim(const netCDF::NcVar &var, const std::vector<std::string> &expected_dims, bool verbose) {
    ...
    if (var.getDimCount() != expected_dims.size()) {
      if (verbose)
        spdlog::critical("{} polar MUST have 5 dimensions", group_name);
      compliant = false;
    }
  ...
  inline bool check_R6(const netCDF::NcGroup &group, bool verbose) {
    ...
    if (group_name == "PPP" || group_name == "HPPP") {
      if (!group.getVars().contains("TotalBrakePower")) {
        if (verbose)
          spdlog::critical("In group {}, mandatory variable {} not found.", group_name, "TotalBrakePower");
        compliant = false;
      }
    }
    ...
    if (group_name == "HVPP-PB" || group_name == "VPP-PB" || group_name == "VPP") {
      if (!group.getVars().contains("STW")) {
        if (verbose)
          spdlog::critical("In group {}, mandatory variable {} not found.", group_name, "STW");
        compliant = false;
      }
    }
    ...
    if (!group.getVars().contains("LEEWAY")) {
      if (verbose)
        spdlog::critical("In group {}, mandatory variable {} not found.", group_name, "LEEWAY");
      compliant = false;
    }
  ...
  inline bool check_dim_existence(const netCDF::NcGroup &group,
    ...
    for (const auto &dim: expected_dims) {
      if (!group.getDims().contains(dim)) {
        compliant = false;
        if (verbose)
          spdlog::critical("In group {}, mandatory dimension {} not found", group.getName(), dim);
      }
    }
    return compliant;
  }
  ...
  inline bool check_rules(const std::string &nc_polar, bool verbose = true) {
    ...
    if (ncfile.getAtts().contains("file_type")) {
      ...
      if (verbose)
        spdlog::critical("No attribute named file_type");
      compliant = false;
    }
    if (ncfile.getAtts().contains("poem_file_format_version")) {
      ...
      if (verbose)
        spdlog::critical("No attribute poem_file_format_version");
      compliant = false;
    }

    if (!compliant) {
      if (verbose)
        spdlog::critical("Not compliant with v1/R1");
      return false;
    }
    ...
    if (!compliant) {
      if (verbose)
        spdlog::critical("Not compliant with v1/R2");
      return false;
    }
    ...
    if (!compliant) {
      if (verbose)
        spdlog::critical("Not compliant with v1/R3");
      return false;
    }
