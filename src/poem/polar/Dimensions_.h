...
#ifndef POEM_DIMENSIONS__H
#define POEM_DIMENSIONS__H
...
namespace poem {
  ...
  class Dimension_ : public Named {
   public:
     ...
     */
    Dimension_(const std::string &name,
  ...
  /**
   * DimensionSet declares a set of Dimension that define the coordinates of a polar
   * @tparam _dim the number of dimensions of a polar
   */
  template<size_t _dim>
  class DimensionSet {
   public:
    using DimensionArray = std::array<std::shared_ptr<Dimension_>, _dim>;

    explicit DimensionSet(const DimensionArray &array) :
        m_dimension_array(array) {
    }

    bool operator==(const DimensionSet<_dim> &other) const {
      for (size_t i = 0; i < _dim; ++i) {
        if (*m_dimension_array[i] != *other.m_dimension_array[i]) {
          return false;
        }
      }
      return true;
    ...
    Dimension_ *get(size_t i) const { return m_dimension_array.at(i).get(); }

    const size_t get_index(const std::string &name) const {
      for (size_t i = 0; i < _dim; ++i) {
        if (m_dimension_array.at(i)->name() == name) return i;
      }

      std::string available_names = "{";
      for (const auto &dim_ID: m_dimension_array) {
        available_names += dim_ID->name() + "; ";
      }
      available_names += "}";

      spdlog::critical(R"(In DimensionSet, attempting to access to non registered dimension name "{}".
  ...
  class DimensionPoint {
   public:
    using ValueArray = std::array<double, _dim>;

    DimensionPoint(std::shared_ptr<DimensionSet<_dim>> dimension_set, const ValueArray &values) :
        m_values(values),
...
//      Check();
    ...
    const double &get(const std::string &name) {
      return m_values.at(m_dimension_set->get_index(name));
    }
...
//    }
  ...
  class DimensionPointSet : public DimensionPointSetBase {
   public:
    ...
    explicit DimensionPointSet(std::shared_ptr<DimensionSet<_dim>> dimension_ID_set) :
//        m_is_built(false),
        m_grid(nullptr),
        m_dimension_ID_set(dimension_ID_set) {}

    DimensionPointSet(std::shared_ptr<DimensionSet<_dim>> dimension_ID_set,
                      DimensionPointGrid<_dim> *grid) :
        m_grid(grid),
        m_dimension_ID_set(dimension_ID_set) {}

    const DimensionSet<_dim> *dimension_ID_set() const { return m_dimension_ID_set.get(); }

    std::shared_ptr<DimensionSet<_dim>> dimension_ID_set() { return m_dimension_ID_set; }
...
//        dim_point_set->m_is_built = true;
   ...
   private:
    std::shared_ptr<DimensionSet<_dim>> m_dimension_ID_set;
  ...
  class DimensionPointGrid {
   public:
    explicit DimensionPointGrid(std::shared_ptr<DimensionSet<_dim>> dimension_ID_set) :
        m_is_built(false),
        m_dimension_ID_set(dimension_ID_set) {}
   ...
   private:
    ...
    void build() {
      ...
      typename DimensionPoint<_dim>::ValueArray values; // ValueArray :: array<double, _dim>
    ...
    constexpr void meta_for_loop(DimensionPointVector &dimension_points,
                                 typename DimensionPoint<_dim>::ValueArray &values) {
