...
class DimensionPointSetTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    // Create Dimensions
    auto STW_dim = std::make_shared<Dimension>("STW_kt", "kt", "Speed Through Water", STW_vector);
    auto TWS_dim = std::make_shared<Dimension>("TWS_kt", "kt", "True Wind Speed", TWS_vector);
    auto TWA_dim = std::make_shared<Dimension>("TWA_deg", "deg", "True Wind Angle", TWA_vector);
    auto WA_dim = std::make_shared<Dimension>("WA_deg", "deg", "Waves Angle", WA_vector);
    auto Hs_dim = std::make_shared<Dimension>("Hs_m", "m", "Waves Significant Height", Hs_vector);

    // Create a DimensionSet
    DimensionArray<5> dimension_array{{STW_dim, TWS_dim, TWA_dim, WA_dim, Hs_dim}};

    dimension_point_set = std::make_shared<DimensionPointSet<5>>(dimension_array);
  }
  ...
  std::shared_ptr<DimensionPointSet<5>> dimension_point_set;
