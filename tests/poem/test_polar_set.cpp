...
//    attributes.add_attribute("name", "PPP");
  auto polar = std::make_shared<poem::Polar>(polar_name, attributes, poem::POLAR_TYPE::PPP);
  polar->new_polar_table<double, 5>("BrakePower", "kW", "Brake Power", poem::type::POEM_TYPES::DOUBLE,
                                    dimension_point_set);
  //(polar_table, "MylittlePolar");

  std::cout << "     PolarSet creation " << std::endl;
  poem::Attributes polar_set_attributes;
  polar_set_attributes.add_attribute("beam_m", "16.500000");
  polar_set_attributes.add_attribute("copyright", "D-ICE ENGINEERING (C)");
  polar_set_attributes.add_attribute("design_speed_kt", "8");
  polar_set_attributes.add_attribute("displacement_tons", "4643");
  polar_set_attributes.add_attribute("max_brake_power_kW", "9999");
  polar_set_attributes.add_attribute("name", "MyLittleShip");
  polar_set_attributes.add_attribute("poem_file_format_version", "v1");
  polar_set_attributes.add_attribute("vessel_type", "HYBRID");
  auto polar_set = std::make_shared<poem::PolarSet>("/", polar_set_attributes);
  ...
  for (auto pl: all_polar_type_list) {
    ...
    std::cout << polar_set->exist(pl) << std::endl;
  }

  std::vector<POLAR_TYPE> res = polar_set->polar_type_list();
  ...
  std::cout << "Construction of Polar from the polar_table function" << std::endl;
  // construct polars
  for (auto polar_name: res) {
    std::shared_ptr<Polar> ps_ptr = polar_set->polar(polar_name);
    ...
    std::cout << ")" << std::endl;
    for (auto p: polar_names) {
      std::shared_ptr<InterpolablePolarTable<5>> polar_table_ = ps_ptr->polar_table<double, 5>(p);
